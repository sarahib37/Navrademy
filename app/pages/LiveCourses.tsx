        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="space-y-12">
            {courses.map((course, i) => (
              <motion.div key={course.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="accent-card p-8 lg:p-10">
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  ...
                  <span className="flex items-center gap-1 text-sm text-muted-foreground"><Users className="h-4 w-4" />{course.students} students</span>
                </div>
                ...
                <Button variant="hero" size="lg" asChild>
                  <Link to="/waitlist">Apply Now <ArrowRight className="h-4 w-4" /></Link>
                </Button>
              </motion.div>
            ))}
          </div>
