            ...
            {programs.map((p, i) => (
              <motion.div key={p.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="elevated-card p-8 group flex flex-col">
                ...
                <div className="mb-6 flex-1">
                  ...
                  <ul className="space-y-2">{p.outcomes.map((o) => <li key={o} className="flex items-start gap-2 text-sm text-muted-foreground"><CheckCircle className="h-4 w-4 text-primary mt-0.5 shrink-0" />{o}</li>)}</ul>
                </div>
                <Button variant="hero" className="w-full" asChild>
                  <Link to="/waitlist">Enroll Now <ArrowRight className="h-4 w-4" /></Link>
                </Button>
              </motion.div>
            ))}
          </div>
