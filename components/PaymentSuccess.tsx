// app/courses/payment-success/PaymentSuccessClient.tsx
"use client";
...
export default function PaymentSuccessClient({ courseProp }: { courseProp?: string }) {
  const searchParams = useSearchParams();
  const router = useRouter();

  const reference = searchParams.get("reference");
  const course = courseProp || searchParams.get("course") || "Your Course";
  ...
  useEffect(() => {
    ...
    const verifyPayment = async () => {
      try {
        const res = await fetch(`/courses/api/paystack/verify?reference=${reference}`);
